...
const resolvers = {
    ...
    Mutation: {
      createProduct: async (_, { input }, { db }) => {
        try {
          const productsCollection = db.collection('products'); // Replace 'products' with your actual collection name
  
          // Insert the new product into the collection
          const result = await productsCollection.insertOne(input);
  
          // If the insertion was successful, return the newly created product
      ...
      updateProduct: async (_, { id, input }, { db }) => {
        try {
          // Assuming 'products' is your MongoDB collection
          const objectId = new ObjectId(id);
          const existingProduct = await Product.findOne({ _id: objectId });
  
          if (!existingProduct) {
            throw new Error(`Product with _id ${id} not found.`);
          }
  
          // Update only the fields that are present in the input
          const updatedFields = { ...existingProduct, ...input };
          ...
          if (result.modifiedCount === 1) {
            ...
            throw new Error(`Error updating product with _id ${id}.`);
          }
        } catch (error) {
